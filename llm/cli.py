...
def prompt(
    ...
    if save:
        ...
        if tools_debug_mode:
            to_save["tools_debug_mode"] = tools_debug_mode
        if tools_approve:
            to_save["tools_approve"] = True
        if ctx.get_parameter_source("chain_limit") != ParameterSource.DEFAULT:
    ...
    if template:
        ...
        if template_obj.chain_limit is not None and ctx.get_parameter_source('chain_limit') == ParameterSource.DEFAULT:
...
def chat(
    ...
    if template_obj and template_obj.tools:
